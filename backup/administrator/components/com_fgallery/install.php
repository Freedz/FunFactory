<?php
/*------------------------------------------------------------------------
# 1 Flash Gallery content plugin for Joomla!
# ------------------------------------------------------------------------
...
require_once (JPATH_ROOT.DS.'administrator'.DS.'components'.DS.'com_installer'.DS.'models'.DS.'components.php');
$c = new InstallerModelComponents ();
$c->_loadItems();


foreach ($c->_items as $item){
    
    if ($item->name == 'FGallery'){
        
        require_once ($this->parent->getPath('source').DS.'update.php');
        
        if(version_compare($item->version, '1.30', '<')){
            
            fgallery_update::db_update_to_130();
...
if (is_a($plugins, 'JSimpleXMLElement') && count($plugins->children())) {
	...
	$path = JPATH_ROOT.DS.'images'.DS.'stories'.DS.'fgallery';
	if (!file_exists($path)) {
...
<h2>1 Flash Gallery Installation</h2>
<table class="adminlist">
	...
	<tbody>
		<tr class="row0">
			<td class="key" colspan="2"><?php echo '1 Flash Gallery '.JText::_('Component'); ?></td>
