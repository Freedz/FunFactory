...
*/
	...
	if ($member_id != 0){
		...
		if ($modSettings['2sichat_permissions']) {
	...
	if ($modSettings['2sichat_permissions'] && !$permission['2sichat_access'] && !$permission['is_admin'] && !$permission['is_mod']) {
		$context['JSON']['STATUS'] = 'NO ACCESS'; // Sorry but you don't have access
		doOutput();
	} else if ($modSettings['2sichat_permissions'] && !$permission['is_admin'] && !$permission['is_mod']) {
