...
/*
	...
	void aeva_tagHelper()
		- Shows the complete popup help page for the [smg] BBCode tag
...
function aeva_moveItems()
{
	...
	global $amFunc, $context, $scripturl, $galurl, $txt, $user_info, $amSettings;
	...
	$request = $amFunc['db_query']('
		SELECT
			m.id_media, m.title, m.id_member, IFNULL(mem.realName, m.member_name) AS member_name, a.name, a.id_album, m.id_file, m.type,
		...
		array('media' => array_values($ids)),__FILE__,__LINE__);
	...
	foreach ($context['aeva_album_list'] as $k => $list)
	{
		...
		$albums[$list] = array(str_repeat('-', $context['aeva_albums'][$list]['child_level']).' '.$context['aeva_albums'][$list]['name'], false, $list == $items[0]['id_album'] ? ' disabled="disabled"' : null);
	}
	$albums['sep' . ++$sep] = array('', false, '');

	// Load up the form
	$context['aeva_form_headers'] = array(
		array($txt['aeva_moving_item'] . ': ' . $moving_link),
		array($txt['aeva_album'] . ': <a href="' . $galurl . 'sa=album;in=' . $items[0]['id_album'] . '">' . $items[0]['name'] . '</a>'),
	);
	...
	if (isset($_POST['submit_aeva']))
	{
		...
		if (!isset($albums[$album_id]))
			fatal_lang_error('aeva_invalid_album');
		...
		foreach ($items as $i)
		{
			...
			if ($i['id_thumb'] > 4 && file_exists($amSettings['data_dir_path'].'/'.$i['thumb_dir'].'/'.aeva_getEncryptedFilename($i['thumb_file'], $i['id_thumb'], true)))
			{
				...
				if (!$success)
					fatal_lang_error('aeva_filemove_failed');
				$amFunc['db_query']('
					UPDATE {db_prefix}aeva_files
					SET id_album = {int:album}, directory = {string:dir}
					WHERE id_file = {int:file}',
					array('file' => $i['id_thumb'], 'dir' => $_new_dir, 'album' => $album_id),__FILE__,__LINE__);
			}
			if ($i['id_preview'] > 4 && file_exists($amSettings['data_dir_path'].'/'.$i['preview_dir'].'/'.aeva_getEncryptedFilename($i['preview_file'], $i['id_preview'])))
			{
				...
				$amFunc['db_query']('
					...
					SET id_album = {int:album}, directory = {string:dir}
					WHERE id_file = {int:file}',
					array('file' => $i['id_preview'], 'dir' => $_new_dir, 'album' => $album_id),__FILE__,__LINE__);
			}
			if ($i['id_file'] != 0 && file_exists($main_file))
			{
				...
				$amFunc['db_query']('
					...
					SET id_album = {int:album}, directory = {string:dir}
					WHERE id_file = {int:file}',
					array('file' => $i['id_file'], 'dir' => $_new_dir, 'album' => $album_id),__FILE__,__LINE__);
			}
			...
			$amFunc['db_query']('
				...
				array('album' => $album_id, 'media' => $i['id_media']),__FILE__,__LINE__);
			...
			$amFunc['db_query']('
				...
				array('album' => $i['id_album']),__FILE__,__LINE__);
			$amFunc['db_query']('
				UPDATE {db_prefix}aeva_albums
	...
	foreach ($albums as $a)
		$amFunc['add_linktree']($galurl . 'sa=album;in=' . $a['id'], $a['name']);
	$amFunc['add_linktree']($galurl . 'sa=item;in=' . $items[0]['id_media'], $items[0]['title']);
	$amFunc['add_linktree']($galurl . 'sa=move;in=' . $items[0]['id_media'], $txt['aeva_moving']);
...
function aeva_unseen()
{
	...
	global $galurl, $scripturl, $amSettings, $amFunc, $txt, $user_info, $context;
	...
	if (!empty($_REQUEST['pageseen']))
	{
		...
		foreach ($pageseen as $item)
			if ((int) $item > 0)
				aeva_markSeen((int) $item, 'force_insert');
	...
	// Now get the items!
	$context['aeva_items'] = aeva_fillMediaArray($amFunc['db_query']('
		SELECT
		...
		FROM {db_prefix}aeva_media AS m
			INNER JOIN {db_prefix}aeva_albums AS a ON (m.album_id = a.id_album)
			LEFT JOIN {db_prefix}aeva_files AS f ON (f.id_file = m.id_thumb)
		...
		array('start' => $start, 'per_page' => $per_page, 'album' => $album, 'user' => $user_info['id']),__FILE__,__LINE__));
	...
	if (!empty($comment_list))
	{
		...
		$request = $amFunc['db_query']('
			...
			FROM {db_prefix}aeva_comments AS c
				LEFT JOIN {db_prefix}aeva_log_media AS lm ON (lm.id_media = c.id_media AND lm.id_member = {int:user})
				LEFT JOIN {db_prefix}aeva_log_media AS lm_all ON (lm_all.id_media = 0 AND lm_all.id_member = {int:user})
	...
	$context['aeva_page_index'] = $amFunc['construct_page_index']($galurl . 'sa=unseen' . (isset($_REQUEST['album']) ? ';album=' . (int) $_REQUEST['album'] : ''), $start, $total_items, $per_page);

	// Sub template and page title
	$context['aeva_header']['data']['title'] = $txt['aeva_viewing_unseen'];
	$context['page_title'] = $txt['aeva_viewing_unseen'];
	$context['sub_template'] = 'aeva_unseen';
	$context['aeva_current'] = 'unseen';
	$amFunc['add_linktree']($galurl.'sa=unseen',$txt['aeva_viewing_unseen']);
}

// Handles searching of items
function aeva_mgSearch()
{
	global $galurl, $txt, $amFunc, $amSettings, $user_info, $context, $scripturl, $sourcedir;
	...
	// Search linktree
	$amFunc['add_linktree']($galurl.'sa=search', $txt['aeva_search']);

	// Page title
	$context['page_title'] = $txt['aeva_search'];
	...
	if (isset($_REQUEST['search']))
	{
		if (trim($_REQUEST['search']) == '')
			fatal_lang_error('aeva_search_left_empty', false);
		...
		$filters = array(
			...
			'member' => isset($_REQUEST['sch_mem']) && trim($_REQUEST['sch_mem']) != '' ? $amFunc['htmlspecialchars']($_REQUEST['sch_mem']) : false,
		...
		$as_playlist = !empty($context['current_board']) && $context['aeva_foxy'];
		$test = false;
		...
		if ($test === false)
			fatal_lang_error('aeva_no_search_option_selected', false);
		...
		if ($filters['member'])
		{
			...
			$members = strtr(addslashes($amFunc['htmlspecialchars'](stripslashes($filters['member']), ENT_QUOTES)), array('&quot;' => '"'));
			...
			$request = $amFunc['db_query']('
				...
				WHERE realName IN ({string:name}) OR memberName IN ({string:name})',
				array('name' => implode(',',$who)),__FILE__,__LINE__);
			while ($row = $amFunc['db_fetch_assoc']($request))
			...
			if (!empty($members_to_filter))
			{
				...
				fatal_lang_error('aeva_search_mem_not_found',false);
		}
		if ($filters['album'])
		{
		...
		if (!empty($filters['custom_fields']))
		{
			foreach ($filters['custom_fields'] as $k => $v)
			{
				...
				$key = 'cfd' . $v;
				$joins[] = 'LEFT JOIN {db_prefix}aeva_field_data AS ' . $key . ' ON (' . $key . '.id_field = ' . $v . ' AND ' . $key . '.id_media = m.id_media)';
		...
		$context['aeva_items'] = aeva_fillMediaArray($amFunc['db_query']('
			SELECT
				...
				IFNULL(mem.realName, m.member_name) AS member_name, a.name, a.id_album, m.time_added,
				IFNULL(lm.time, IFNULL(lm_all.time, 0)) < m.log_last_access_time AS is_new, f.width, f.height,
				m.id_thumb, f.directory AS thumb_dir, f.filename AS thumb_file
			FROM {db_prefix}aeva_media AS m
				INNER JOIN {db_prefix}aeva_albums AS a ON (m.album_id = a.id_album)
				LEFT JOIN {db_prefix}aeva_files AS f ON (f.id_file = m.id_thumb)
				LEFT JOIN {db_prefix}members AS mem ON (mem.ID_MEMBER = m.id_member)
				LEFT JOIN {db_prefix}aeva_log_media AS lm ON (lm.id_media = m.id_media AND lm.id_member = {int:user})
				LEFT JOIN {db_prefix}aeva_log_media AS lm_all ON (lm_all.id_media = 0 AND lm_all.id_member = {int:user})' . (!empty($joins) ? '
				' . implode('
				', $joins) : '') . '
			WHERE {query_see_hidden_albums} AND (' . $query . ')' . (!aeva_allowedTo('moderate') ? '
			AND m.approved = 1' : '') . '
			...
			'search' => $searching_for, 'start' => isset($_REQUEST['start']) ? (int) $_REQUEST['start'] : 0, 'per_page' => $as_playlist ? 30 : 15),__FILE__,__LINE__));

		// Pagination query
		$request = $amFunc['db_query']('
			SELECT COUNT(m.id_media)
			FROM {db_prefix}aeva_media AS m
				INNER JOIN {db_prefix}aeva_albums AS a ON (m.album_id = a.id_album)' . (!empty($joins) ? implode('
				', $joins) : '') . '
			WHERE {query_see_hidden_albums} AND (' . $query . ')' . (!aeva_allowedTo('moderate') ? '
			AND m.approved = 1' : ''),
			array('user' => $user_info['id'], 'mem' => !empty($members_to_filter) ? implode(' OR m.id_member = ',$members_to_filter) : '',
			'album' => $filters['album'], 'search' => $searching_for),__FILE__,__LINE__);
		list ($total_items) = $amFunc['db_fetch_row']($request);
		...
		$start = isset($_REQUEST['start']) ? (int) $_REQUEST['start'] : 0;
		$context['aeva_page_index'] = $amFunc['construct_page_index']($pageindexURL, $start, $total_items, $as_playlist ? 30 : 15);
		...
		// Sub template
		$context['sub_template'] = 'aeva_search_results';
		$context['aeva_searching_for'] = $amFunc['htmlspecialchars']($_REQUEST['search']);
		$context['aeva_total_results'] = $total_items;

		// Foxy! -- if we're in a board, means we're viewing a playlist... Then return a playlist.
		if ($as_playlist)
			aeva_foxy_search();
	}
	else
	{
		$context['sub_template'] = 'aeva_search_searching';
...
function aeva_listAlbums()
{
	...
	global $txt, $amFunc, $amSettings, $user_info, $scripturl, $galurl, $context;
	...
	$total_items = $amFunc['db_num_rows']($request);
	$amFunc['db_free_result']($request);
	...
	if (isset($_REQUEST['asc']))
		$pageindexURL .= ';asc';
	$context['aeva_page_index'] = $amFunc['construct_page_index']($pageindexURL, $start, $total_items, $per_page);

	// End this
	$context['sub_template'] = 'aeva_viewUserAlbums';
	$amFunc['add_linktree']($galurl . 'sa=vua', $txt['aeva_albums']);
	$context['aeva_header']['data']['title'] = $txt['aeva_albums'];
...
function aeva_mgStats()
{
	...
	global $amFunc, $amSettings, $user_info, $context, $txt, $galurl;
	...
	********************************************************/
	...
	$total_item_contributors = $amFunc['db_num_rows']($request);
	$amFunc['db_free_result']($request);
	...
	$request = $amFunc['db_query']('
		...
		WHERE approved = 1
		GROUP BY id_member',array(),__FILE__,__LINE__);
	$total_comment_contributors = $amFunc['db_num_rows']($request);
	$amFunc['db_free_result']($request);
	...
	$request = $amFunc['db_query']('
		...
		WHERE approved = 1
		AND featured = 1',array(),__FILE__,__LINE__);
	list ($total_featured_albums) = $amFunc['db_fetch_row']($request);
	$amFunc['db_free_result']($request);
	...
	$context['aeva_header']['data']['title'] = $txt['aeva_stats'];
	$context['page_title'] = $txt['aeva_gallery'].' - '.$txt['aeva_stats'];
	$context['aeva_current'] = 'stats';
	$context['sub_template'] = 'aeva_stats';
	$amFunc['add_linktree']($galurl.'sa=stats', $txt['aeva_stats']);
}

// Manages your albums' control panel
function aeva_albumCP($is_admin = false)
{
	global $amFunc, $user_info, $context, $txt, $galurl, $alburl, $scripturl, $settings;
	...
	<script type="text/javascript" language="javascript" src="' . aeva_theme_url('admin.js') . '"></script>
	<script type="text/javascript"><!-- // --><![CDATA[
		var galurl = "' . $galurl . '";
	// ]]></script>';

		$context['page_title'] = $txt['aeva_my_user_albums'];
		$context['aeva_header']['data']['title'] = $txt['aeva_my_user_albums'];

		$amFunc['add_linktree']($galurl . 'area=mya', $txt['aeva_my_user_albums']);
	}
	...
	// Sub-Template
	$context['sub_template'] = 'aeva_album_cp';
	...
	foreach ($context['aeva_album_list'] as $list)
	{
		...
		if ($moving !== false && $moving != $list)
		{
			...
			$context['aeva_my_albums'][$list]['move_links'] = array(
				'before' => sprintf($move_url, 'before', 'up', $txt['aeva_admin_before']),
				'after' => sprintf($move_url, 'after', 'down', $txt['aeva_admin_after']),
				'child_of' => sprintf($move_url, 'child', 'in', $txt['aeva_admin_child_of']),
			);
...
function aeva_getFromMemberlist($lis, $owner = 1)
{
	...
	$request = $amFunc['db_query']('
		SELECT
			ID_MEMBER AS id_member, ' . ($owner ? 'member' : 'real') . 'Name
		FROM {db_prefix}members' . (is_numeric($lis[0]) ? '
		WHERE ID_MEMBER IN ({array_int:names})' : '
		WHERE realName IN ({array_string:names}) OR memberName IN ({array_string:names})') . '
...
{
	global $context, $scripturl, $sourcedir, $galurl, $amFunc, $amSettings, $txt, $user_info, $settings;
	...
	$request = $amFunc['db_query']('SELECT ID_GROUP FROM {db_prefix}members GROUP BY ID_GROUP ORDER BY ID_GROUP', array(),__FILE__,__LINE__);
	while ($row = $amFunc['db_fetch_row']($request))
	...
	else
	{
		...
		$request = $amFunc['db_query']('
			SELECT
				...
				a.access, a.access_write, a.allowed_members, a.allowed_write, a.denied_members, a.denied_write, mem.memberName, mem.realName
			FROM {db_prefix}aeva_albums AS a
				LEFT JOIN {db_prefix}aeva_files AS f ON (f.id_file = a.icon)
				LEFT JOIN {db_prefix}aeva_files AS bf ON (bf.id_file = a.bigicon)
			...
			LIMIT 1',
			array('album' => (int) $_GET['in']),__FILE__,__LINE__);
		if ($amFunc['db_num_rows']($request) == 0)
		...
		if ($id_topic != $id_topic2)
		{
			$id_topic = 0;
			$amFunc['db_query']('
				UPDATE {db_prefix}aeva_albums SET id_topic = 0 WHERE id_album = {int:album}', array('album' => (int) $_GET['in']), __FILE__,__LINE__);
		}
	}

	require_once($sourcedir . '/Subs-Post.php');
	$title = un_preparsecode($title);
	$description = un_preparsecode($description);
	...
	$groups = array();
	$groups[-1] = array($txt['aeva_membergroups_guests'], in_array(-1, $access));
	$groups[0] = array('<span' . (isset($primary_groups[0]) && !$simple_groups ? ' style="font-weight: bold" title="' . $txt['aeva_admin_membergroups_primary'] . '"' : '') . '>' . $txt['aeva_membergroups_members'] . '</span>', in_array(0, $access), in_array(0, $access_write));
	$groups['sep1'] = 'sep';
	...
	else
	{
		...
		while ($row = $amFunc['db_fetch_assoc']($request))
		{
			...
			$groups[$row['id']] = array(
				'<span' . ($row['is_post_group'] ? ' style="border-bottom: 1px dotted;" title="' . $txt['mboards_groups_post_group'] . '"' :
					(isset($primary_groups[$row['id']]) ? ' style="font-weight: bold" title="' . $txt['aeva_admin_membergroups_primary'] . '"' : ''))
		...
		$groups['sep3'] = 'sep';
		$groups['check_all'] = array('<i>' . $txt[$context['is_smf2'] ? 'check_all' : 737] . '</i>', 'custom' => 'onclick="invertAll(this, this.form, \'$1[]\');"');
	}
	...
	{
		$sort_fields[$tmp.' DESC'] = array($txt['aeva_sort_by_'.$counter] . ' (' . $txt['aeva_sort_order_desc'] . ') - ' . $sort_help[$counter*2] . '...', $peralbum['sort'] == $tmp.' DESC');
		$sort_fields[$tmp.' ASC'] = array($txt['aeva_sort_by_'.$counter] . ' (' . $txt['aeva_sort_order_asc'] . ') - ' . $sort_help[$counter*2+1] . '...', $peralbum['sort'] == $tmp.' ASC');
	}
	...
	$context['aeva_form'] = array(
		'note' => array(
			'label' => $txt['aeva_album_will_be_approved'],
			'class' => 'windowbg',
			'perm' => $will_be_unapproved,
			'type' => 'title',
		),
		'main' => array(
			'label' => $txt['aeva_album_mainarea'],
			'type' => 'title',
		),
		'name' => array(
			'label' => $txt['aeva_name'],
		...
		'owner_change' => array(
			...
			'next' => '<a href="' . $scripturl . '?' . ($is_admin ? '' : 'action=media;') . 'action=findmember;input=change_owner;delim=null;' . $context['session_var'] . '=' . $context['session_id'] . '" onclick="return reqWin(this.href, 350, 400);"><img alt="" src="' . $settings['images_url'] . '/icons/assist.gif" /></a>',
		),
		'featured' => array(
			'label' => $txt['aeva_featured_album'],
			'subtext' => '', // Temp
			'fieldname' => 'featured',
			'type' => 'yesno',
			'value' => $featured,
		...
		'icon' => array(
			...
			'type' => 'file',
			'fieldname' => 'icon',
			'subtext' => $is_edit ? $txt['aeva_admin_icon_edit_subtext'] : '',
			'add_text' => $is_edit ? '<p><img alt="" src="'.$galurl.'sa=media;in='.$id_album.';icon" style="padding-left: 4px" /></p>' : '',
		),
		'profile' => array(
			'label' => $txt['aeva_perm_profile'],
			'fieldname' => 'profile',
			'options' => $profs,
			'type' => 'select',
		),
		'quotaProfile' => array(
			'label' => $txt['aeva_quota_profile'],
		...
		'position' => array(
			'label' => $txt['aeva_admin_position'],
			'type' => 'select',
			'fieldname' => 'pos',
			'options' => array(
				'before' => $txt['aeva_admin_before'],
				'after' => $txt['aeva_admin_after'],
				'child' => $txt['aeva_admin_child_of'],
			),
		),
		'target' => array(
			'label' => $txt['aeva_admin_target'],
			'fieldname' => 'target',
			'options' => $albums,
			'type' => 'select',
		),
		'topic_board' => array(
			'label' => $txt['aeva_linked_topic_board'],
			'type' => 'select',
			'fieldname' => 'topic_board',
			'options' => $topic_boards,
		),
		'linked_topic' => array(
			'label' => $txt['aeva_linked_topic'],
			'type' => 'link',
			'text' => $topic_subject,
			'link' => $scripturl . '?topic=' . $id_topic . '.0',
		),
		'sort_order' => array(
			'label' => $txt['aeva_default_sort_order'],
			'type' => 'select',
			'fieldname' => 'sort_order',
			'options' => $sort_fields,
		),
		'default_view' => array(
			'label' => $txt['aeva_items_view'],
			'type' => 'select',
			'fieldname' => 'default_view',
			'options' => array(
				'normal' => array($txt['aeva_view_normal'], !empty($peralbum['view']) && $peralbum['view'] == 'normal'),
				'filestack' => array($txt['aeva_view_filestack'], !empty($peralbum['view']) && $peralbum['view'] == 'filestack'),
			),
		),
		'privacy' => array(
			'label' => $txt['aeva_album_privacy'],
			'type' => 'title',
		),
		'groups' => array(
			'label' => $txt['aeva_admin_membergroups'],
			'subtext' => $simple_groups ? preg_replace('~<li>.*?</li>~', '', $txt['aeva_admin_membergroups_subtxt'], 1) : $txt['aeva_admin_membergroups_subtxt'],
		...
		'allowed_members' => array(
			'label' => $txt['aeva_allowed_members'],
			'subtext' => $txt['aeva_allowed_members_subtxt'],
			'type' => 'text',
			'fieldname' => 'allowed_members',
			'value' => empty($members_allowed) ? '' : implode(', ', $members_allowed),
			'custom' => 'maxlength="52" id="allowed_members"',
			'next' => '<a href="' . $scripturl . '?' . ($is_admin ? '' : 'action=media;') . 'action=findmember;input=allowed_members;' . $context['session_var'] . '=' . $context['session_id'] . '" onclick="return reqWin(this.href, 350, 400);"><img alt="" src="' . $settings['images_url'] . '/icons/assist.gif" /></a>',
		),
		'allowed_write' => array(
			'label' => $txt['aeva_allowed_write'],
			'subtext' => $txt['aeva_allowed_write_subtxt'],
			'type' => 'text',
			...
			'next' => '<a href="' . $scripturl . '?' . ($is_admin ? '' : 'action=media;') . 'action=findmember;input=allowed_write;' . $context['session_var'] . '=' . $context['session_id'] . '" onclick="return reqWin(this.href, 350, 400);"><img alt="" src="' . $settings['images_url'] . '/icons/assist.gif" /></a>',
		),
		'denied_members' => array(
			'label' => $txt['aeva_denied_members'],
			'subtext' => $txt['aeva_denied_members_subtxt'],
			'type' => 'text',
			'fieldname' => 'denied_members',
			'value' => empty($members_denied) ? '' : implode(', ', $members_denied),
			'custom' => 'maxlength="52" id="denied_members"',
			'next' => '<a href="' . $scripturl . '?' . ($is_admin ? '' : 'action=media;') . 'action=findmember;input=denied_members;' . $context['session_var'] . '=' . $context['session_id'] . '" onclick="return reqWin(this.href, 350, 400);"><img alt="" src="' . $settings['images_url'] . '/icons/assist.gif" /></a>',
		),
		'denied_write' => array(
			'label' => $txt['aeva_denied_write'],
			'subtext' => $txt['aeva_denied_write_subtxt'],
			'type' => 'text',
			'fieldname' => 'denied_write',
			'value' => empty($members_denied_write) ? '' : implode(', ', $members_denied_write),
			'custom' => 'maxlength="52" id="denied_write"',
			'next' => '<a href="' . $scripturl . '?' . ($is_admin ? '' : 'action=media;') . 'action=findmember;input=denied_write;' . $context['session_var'] . '=' . $context['session_id'] . '" onclick="return reqWin(this.href, 350, 400);"><img alt="" src="' . $settings['images_url'] . '/icons/assist.gif" /></a>',
		),
		'hidden' => array(
			'label' => $txt['aeva_album_hidden'],
			'subtext' => $txt['aeva_album_hidden_subtxt'],
			'fieldname' => 'hidden',
			'type' => 'yesno',
			'value' => $hidden,
		),
		'passwd' => array(
			...
			'subtext' => $txt['aeva_admin_passwd_subtxt'],
			'fieldname' => 'passwd',
			'type' => 'text',
			'value' => preg_match('/^[a-f0-9]{40}$/', $passwd) ? '' : $passwd,
		),
		'lightbox_section' => array(
			'label' => $txt['aeva_lightbox_section'],
			'type' => 'title',
		),
		'lightbox_enable' => array(
			'label' => $txt['aeva_lightbox_enable'],
			'subtext' => $txt['aeva_lightbox_enable_info'],
			'fieldname' => 'lightbox',
			'type' => 'yesno',
			'value' => $peralbum['lightbox'] == 'yes',
		),
		'lightbox_outline' => array(
			'label' => $txt['aeva_lightbox_outline'],
			'subtext' => $txt['aeva_lightbox_outline_info'],
		...
		'lightbox_expand' => array(
			'label' => $txt['aeva_lightbox_expand'],
			'subtext' => $txt['aeva_lightbox_expand_info'],
			'fieldname' => 'lightbox_expand',
			'value' => $peralbum['expand'],
			'type' => 'text',
		),
		'lightbox_autosize' => array(
			'label' => $txt['aeva_lightbox_autosize'],
			'subtext' => $txt['aeva_lightbox_autosize_info'],
			'fieldname' => 'lightbox_autosize',
			'type' => 'yesno',
			'value' => $peralbum['autosize'] == 'yes',
		),
		'lightbox_fadeinout' => array(
			'label' => $txt['aeva_lightbox_fadeinout'],
			'subtext' => $txt['aeva_lightbox_fadeinout_info'],
	...
	if (!$context['aeva_foxy'] || !$id_topic)
		unset($context['aeva_form']['linked_topic']);
	if (!$context['aeva_foxy'] || $id_topic)
		unset($context['aeva_form']['topic_board']);
	...
	if (isset($_POST['submit_aeva']))
	{
		...
		$name = $amFunc['htmlspecialchars']($_POST['name']);
		$name = aeva_utf2entities($name, false, 80 + strlen($name) - strlen($_POST['name']), false, false, true, 20, 255);
		if (empty($name))
			fatal_lang_error('aeva_admin_name_left_empty', false);
		$desc = $amFunc['htmlspecialchars'](aeva_utf2entities($_POST['desc'], false, 0));
		preparsecode($name);
		preparsecode($desc);
		...
		if ($is_add)
		{
			...
			if (!empty($_albums) && !isset($_albums[$_POST['target']]))
				fatal_lang_error('aeva_admin_invalid_target');
			if (!empty($_albums))
			...
			if (!in_array($_POST['pos'], array('before', 'after', 'child')))
				fatal_lang_error('aeva_admin_invalid_position');
		...
		if (isset($_FILES['icon']) && !empty($_FILES['icon']['name']))
		{
			...
			if ($is_edit && !empty($icon) && $icon > 4)
			{
				...
				$request = $amFunc['db_query']('
					...
					LIMIT 1',array('file' => $icon),__FILE__,__LINE__);
				$can_delete_icon = $amFunc['db_num_rows']($request) == 0;
				$amFunc['db_free_result']($request);
				...
				if ($can_delete_icon)
				{
					...
					$amFunc['db_query']('
						...
						WHERE id_file = {int:file}',
						array('file' => $icon),__FILE__,__LINE__);
				}
			}
			if ($is_edit && !empty($big_icon) && $big_icon > 0)
			{
				...
				$request = $amFunc['db_query']('
					...
					FROM {db_prefix}aeva_files AS f
						LEFT JOIN {db_prefix}aeva_media AS m ON (f.id_file = m.id_preview) OR (f.id_file = m.id_file)
					WHERE f.id_file = {int:file}
					LIMIT 1',array('file' => $big_icon),__FILE__,__LINE__);
				list ($can_delete_icon, $big_directory) = $amFunc['db_fetch_row']($request);
				$amFunc['db_free_result']($request);
			...
			$icon_file = new aeva_media_handler;
			...
			$icon = $amFunc['db_insert_id']('{db_prefix}aeva_files', 'id_file');
			...
			if ($thumbnailed)
			{
				...
				if (!empty($preview_width))
				{
					...
					$big_icon = $amFunc['db_insert_id']('{db_prefix}aeva_files', 'id_file');
		...
		if ($is_add)
		{
			...
			// Insert the album
			$amFunc['db_insert'](
				'{db_prefix}aeva_albums',
				$ins_var,
				$ins_val,
				__FILE__,__LINE__
			);
			$id_album = $amFunc['db_insert_id']('{db_prefix}aeva_albums', 'id_album');

			// Insert master ID (i.e. topmost album in the tree)
			if (!empty($parent))
			{
				$request = $amFunc['db_query']('SELECT master FROM {db_prefix}aeva_albums WHERE id_album = {int:parent}', array('parent' => $parent),__FILE__,__LINE__);
				list ($master) = $amFunc['db_fetch_row']($request);
				$amFunc['db_free_result']($request);
				if (empty($master))
					$master = $parent;
			}
				...
				$master = $id_album;
			$amFunc['db_query']('UPDATE {db_prefix}aeva_albums SET master = {int:master} WHERE id_album = {int:album}', array('master' => $master, 'album' => $id_album),__FILE__,__LINE__);
			...
			if ($is_dir !== 1 || !aeva_createAlbumDir($id_album))
			{
				...
				$amFunc['db_query']('
					DELETE FROM {db_prefix}aeva_albums
			...
			if ($icon > 4)
				$amFunc['db_query']('
					...
					array('album' => $id_album, 'file' => $icon, 'bigicon' => (int) $big_icon),__FILE__,__LINE__);
		...
		if ($context['aeva_foxy'] && !empty($_POST['topic_board']))
		{
...
function aeva_moveAlbum()
{
	...
	$amFunc['db_query']('
		...
		WHERE id_album = {int:album}',
		array('order' => $order, 'child_level' => $child_level, 'parent' => $parent, 'master' => $master, 'album' => $src['id']),__FILE__,__LINE__);
	...
	if (!empty($children) && is_array($children))
		$amFunc['db_query']('
			...
			array('order' => $order, 'child_level' => abs($child_level_diff), 'master' => $master, 'albums' => $children),__FILE__,__LINE__);
...
{
	global $amFunc, $context, $amSettings, $user_info, $scripturl, $sourcedir;
	...
	$request = $amFunc['db_query']('
		...
		FROM {db_prefix}aeva_albums AS a
			...
			LEFT JOIN {db_prefix}aeva_files AS bf ON (bf.id_file = a.bigicon)
		WHERE a.id_album = {int:album}',
		array('album' => $id),__FILE__,__LINE__);
	if ($amFunc['db_num_rows']($request) == 0)
		return false;
	list ($id_album, $dir, $icon, $big_icon, $owner, $file, $big_file, $big_dir, $approved, $name, $parent, $master) = $amFunc['db_fetch_row']($request);
	$amFunc['db_free_result']($request);
	...
	$request = $amFunc['db_query']('
		...
		array('album' => $id_album),__FILE__,__LINE__);
	$items = array();
	while ($row = $amFunc['db_fetch_assoc']($request))
		$items[] = $row['id_media'];
	$amFunc['db_free_result']($request);
	...
	if ($icon > 4)
	{
		...
		$amFunc['db_query']('
			DELETE FROM {db_prefix}aeva_files
			WHERE id_file = {int:file}', array('file' => $icon),__FILE__,__LINE__);
	}
	...
	if ($big_icon > 0)
	{
		...
		$amFunc['db_query']('
			DELETE FROM {db_prefix}aeva_files
			WHERE id_file = {int:file}', array('file' => $big_icon),__FILE__,__LINE__);
	}

	// Remove album ID from all custom fields. I'm not getting paid for this job.
	$amFunc['db_query']('
		UPDATE {db_prefix}aeva_fields AS cf
	...
	$amFunc['db_query']('
		DELETE FROM {db_prefix}aeva_albums
		WHERE id_album = {int:album}', array('album' => $id_album),__FILE__,__LINE__);
	...
	$amFunc['db_query']('
		...
		WHERE parent = {int:album}', array('album' => $id_album, 'parent' => $parent),__FILE__,__LINE__);
	...
	if ($master == $id_album)
	{
		$amFunc['db_query']('
			UPDATE {db_prefix}aeva_albums
			SET master = {int:none}
			WHERE master = {int:master}', array('none' => 0, 'master' => $id_album),__FILE__,__LINE__);

		$amFunc['db_query']('UPDATE {db_prefix}aeva_albums SET master = id_album WHERE parent = 0', array(),__FILE__,__LINE__);
		$alb = array();
		$continue = true;
		$unstick = 0;
		while ($continue)
		{
			$amFunc['db_query']('
				UPDATE {db_prefix}aeva_albums AS a1, {db_prefix}aeva_albums AS a2
				SET a1.master = a2.master
				WHERE (a1.parent = a2.id_album) AND (a1.master = 0) AND (a2.master != 0)',
				array(),__FILE__,__LINE__);
			$continue = ($amFunc['db_affected_rows']() > 0) && ($unstick++ < 100);
		}
	}
	...
	$amFunc['db_query']('
		...
		WHERE val1 = {int:album}', array('album' => $id_album),__FILE__,__LINE__);
...
{
	global $amFunc, $amSettings, $txt, $context, $user_info, $sourcedir, $settings, $galurl, $modSettings, $cookiename;
	...
	if (!isset($_REQUEST['album']))
		fatal_lang_error('aeva_album_not_specified');

	$_REQUEST['album'] = (int) $_REQUEST['album'];
	$context['page_title'] = $txt['aeva_multi_upload'];
	...
	if ($amFunc['db_num_rows']($request) == 0)
		fatal_lang_error('aeva_album_not_found');
	list ($id_album, $album_name, $id_member_album) = $amFunc['db_fetch_row']($request);
	$amFunc['db_free_result']($request);
	...
	{
		$context['sub_template'] = 'aeva_multiUpload';
		...
		foreach ($context['allowed_types'] as $filetype => $exts)
			$allowed_exts[] = '{description: "' . $txt['aeva_filetype_' . $filetype] . '", extensions: "' . implode(';', $exts) . '"}';
	...
	<script type="text/javascript" src="' . aeva_theme_url('uploader.js') . '"></script>
	<script type="text/javascript"><!-- // --><![CDATA[
		var galurl = "' . $galurl . '";
		YAHOO.util.Event.onDOMReady(function()
		{
			...
			areas["text"] = { "bytes": "' . $txt['aeva_bytes'] . '", "kb": "' . $txt['aeva_kb'] . '", "mb": "' . $txt['aeva_mb'] . '", "cancel": "'
			. $txt['aeva_mass_cancel'] . '", "tl_php": "' . sprintf($txt['aeva_file_too_large_php'], round($max_php_size/1048576, 1)) . '", "tl_quota": "' . $txt['aeva_file_too_large_quota'] . '", "tl_img": "' . $txt['aeva_file_too_large_img'] . '" };
			areas["quotas"] = { ';
		...
		if (!empty($context['allowed_types']))
		{
			foreach ($context['allowed_types'] as $filetype => $exts)
				if (isset($filetypes[$filetype]))
					foreach ($exts as $ext)
						$context['html_headers'] .= '"' . substr(strrchr($ext, '.'), 1) . '": ' . (int) $context['aeva_max_file_size'][$filetypes[$filetype]] . ', ';
			$context['html_headers'] = substr($context['html_headers'], 0, -2);
		}
	...
	// Are we submitting?
	$context['sub_template'] = 'aeva_multiUpload_xml';
	...
	if (empty($_FILES['Filedata']['name']))
		fatal_lang_error('aeva_file_not_specified');
...
{
	global $amFunc, $context, $amSettings, $user_info, $galurl, $sourcedir;
	...
	if (file_exists($sourcedir . '/Aeva-Foxy.php'))
	{
...
{
	global $amFunc, $amSettings, $context, $txt, $sourcedir, $settings, $galurl, $scripturl;
	...
	$request = $amFunc['db_query']('
		...
		FROM {db_prefix}aeva_albums AS a
			LEFT JOIN {db_prefix}aeva_media AS m ON (m.album_id = a.id_album)
	...
	// Sub template
	$context['sub_template'] = 'aeva_profile_summary';
	$context['page_title'] = $txt['aeva_profile_sum_pt'];
}
...
function aeva_profileItems($memID)
{
	...
	$context['page_index'] = $amFunc['construct_page_index']($scripturl . '?action=profile;u=' . $memID . ($context['is_smf2'] ? ';area' : ';sa') . '=aevaitems', $_REQUEST['start'], $context['member']['aeva']['total_items'], 20);

	// Load the items
	$context['aeva_items'] = aeva_getMediaItems((int) $_REQUEST['start'], 20, 'm.id_media DESC', true, array(), 'm.id_member = '.$memID);

	// Sub template
	$context['sub_template'] = 'aeva_profile_viewitems';
	$context['page_title'] = $txt['aeva_profile_viewitems'];
}
...
function aeva_profileComments($memID)
{
	...
	// The page index
	$_REQUEST['start'] = empty($_REQUEST['start']) ? 0 : (int) $_REQUEST['start'];
	$context['page_index'] = $amFunc['construct_page_index']($scripturl . '?action=profile;u=' . $memID . ($context['is_smf2'] ? ';area' : ';sa') . '=aevacoms', $_REQUEST['start'], $context['member']['aeva']['total_comments'], 20);

	// Load the items
	$context['aeva_coms'] = aeva_getMediaComments((int) $_REQUEST['start'], 20, false, array(), 'com.id_member = '.$memID);

	// Sub template
	$context['sub_template'] = 'aeva_profile_viewcoms';
	$context['page_title'] = $txt['aeva_profile_viewcoms'];
}
...
function aeva_profileVotes($memID)
{
	...
	// The page index
	$_REQUEST['start'] = empty($_REQUEST['start']) ? 0 : (int) $_REQUEST['start'];
	$request = $amFunc['db_query']('SELECT COUNT(rating) FROM {db_prefix}aeva_log_ratings WHERE id_member = {int:member}', array('member' => $memID), __FILE__,__LINE__);
	list ($co) = $amFunc['db_fetch_row']($request);
	$amFunc['db_free_result']($request);
	$context['page_index'] = $amFunc['construct_page_index']($scripturl . '?action=profile;u=' . $memID . ($context['is_smf2'] ? ';area' : ';sa') . '=aevavotes', $_REQUEST['start'], $co, 50);

	// Any messages in the associated topics?
	$request = $amFunc['db_query']('
		SELECT DISTINCT COUNT(*) AS co, id_topic FROM {db_prefix}messages
		WHERE ID_MEMBER = {int:id}
		GROUP BY id_topic', array('id' => $memID), __FILE__, __LINE__);
	$nmsg = '';
	while ($row = $amFunc['db_fetch_assoc']($request))
	...
	// Load the ratings
	$req = '
		SELECT p.id_media, p.time, a.name, d.title, p.id_member, p.rating, m.realName AS member_name, d.album_id, a.id_topic
		FROM {db_prefix}aeva_log_ratings AS p
		INNER JOIN {db_prefix}members AS m ON m.id_member = p.id_member
		INNER JOIN {db_prefix}aeva_media AS d ON d.id_media = p.id_media
		INNER JOIN {db_prefix}aeva_albums AS a ON a.id_album = d.album_id
		WHERE p.id_member = {int:member}
		ORDER BY p.rating DESC
		LIMIT {int:start}, {int:limit}';

	$request = $amFunc['db_query']($req,
		array(
			'member' => $memID,
	...
	$request = $amFunc['db_query']('
		SELECT COUNT(p.id_member) AS co, p.id_member, m.realName AS real_name
		FROM {db_prefix}aeva_log_ratings AS p
		INNER JOIN {db_prefix}members AS m ON m.ID_MEMBER = p.id_member
		GROUP BY p.id_member
		ORDER BY co DESC', array(), __FILE__, __LINE__);
	while ($row = $amFunc['db_fetch_assoc']($request))
		$context['aeva_otherVoters'][] = $row;
	$amFunc['db_free_result']($request);

	// Sub template
	$context['sub_template'] = 'aeva_profile_viewvotes';
	$context['page_title'] = $txt['aeva_profile_viewvotes'];
}

// Who rated what?
function aeva_whoRatedWhat()
{
	global $amFunc, $amSettings, $context, $scripturl, $galurl, $txt;
	...
	if (!allowedToAccessItem($context['item']['id_media']))
		fatal_lang_error('aeva_item_access_denied');

	// Load the rating log
	$request = $amFunc['db_query']('
		SELECT lr.id_member, lr.rating, lr.time, mem.realName AS member_name
		FROM {db_prefix}aeva_log_ratings AS lr
	...
	{
		$context['item']['rating_logs'][] = array(
			'member_link' => $scripturl . '?action=profile;u=' . $row['id_member'] . ($context['is_smf2'] ? ';area' : ';sa') . '=aevavotes',
	...
	// Sub template
	$context['sub_template'] = 'aeva_whoRatedWhat';

	$amFunc['add_linktree']($galurl . 'sa=item;in=' . $context['item']['id_media'], $context['item']['title']);
	$amFunc['add_linktree']($galurl . 'sa=whoratedwhat;in=' . $context['item']['id_media'], $txt['aeva_who_rated_what']);
	$context['page_title'] = $txt['aeva_who_rated_what'];
}
...
function aeva_tagHelper()
{
	...
	loadLanguage('Help');
	loadTemplate('Help');
	$context['page_title'] = $txt['aeva_gallery'] . ' - [smg]';
	$context['template_layers'] = array();
	$context['sub_template'] = 'popup';
	$context['help_text'] = $txt['aeva_smg_tag'];
}

// Multi download
function aeva_massDownload()
{
	global $txt, $context, $amSettings, $scripturl, $user_info, $amFunc, $galurl;
	...
	$context['aeva_form'] = array(
		'title' => array(
			'label' => '<input type="hidden" name="album" value="' . $context['aeva_album']['id'] . '" />' . $txt['aeva_multi_download'] . ' - ' . '<a href="' . $galurl . 'sa=album;in=' . $context['aeva_album']['id'] . '">' . $context['aeva_album']['name'] . '</a>',
			'type' => 'title',
		),
		'title2' => array(
			'label' => $txt['aeva_multi_download_desc'],
			'type' => 'title',
			'class' => 'windowbg2',
		),
		'items' => array(
			'label' => $txt['aeva_items'],
			'type' => 'select',
			'multi' => true,
	...
	$context['page_title'] = $txt['aeva_multi_download'];
	$context['sub_template'] = 'aeva_form';
	$amFunc['add_linktree'](
...
{
	global $context, $scripturl, $txt, $user_info, $galurl, $amSettings, $time_start, $amFunc;
	...
	if ($_SESSION['aeva_mdl']['num_done'] < $compte)
	{
		...
		$context['aeva_done_txt'] = sprintf($txt['aeva_multi_dl_wait'], $_SESSION['aeva_mdl']['num_done'], $compte);
		$context['html_headers'] .= '
