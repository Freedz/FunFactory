...
// Media handler class
// This is for various image functions, supporting library such as GD2, IMageMagick, FFMpeg
class aeva_media_handler
{
	...
	{
		global $amSettings, $sourcedir, $context;
			...
			if (!$ffmpeg)
			{
				...
				if ($this->media_type() == 'video' || $this->media_type() == 'audio')
				{
					...
					if (!defined('GETID3_HELPERAPPSDIR'))
					{
						...
						require_once($sourcedir . '/Aeva-getid3/getid3.php');
					}
	...
	{
		global $amSettings, $amFunc;

		if (empty($amSettings['upload_security_check']))
			return;
		if (preg_match('~[\x00-\x1F/\\\\]~', $filename))
			die('Malformed filename... /, \\ or control code found! (' . htmlspecialchars($filename) . ')');
		if (function_exists('file_get_contents'))
		{
			...
			if (preg_match('~<(iframe|\?php|eval|body|head|html|img[^>]+src|plaintext|a[^>]+href|pre|script|table|title)[\s>]~', $scan_file, $hack) === 1)
				die('Hacking attempt... (' . $amFunc['htmlspecialchars']($hack[0]) . ')');
			unset($scan_file);
		}
	}
	...
	{
		global $sourcedir, $amSettings, $context;
		...
		if ($this->image_handler == 1)
		{
			if ($this->createJpgThumbnail($this->src, $width, $height))
	...
	function getInfo()
	{
		...
		elseif ($media_type == 'image')
		{
			...
			require_once($sourcedir . '/Aeva-Exif.php');
